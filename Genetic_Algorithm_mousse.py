from Run_Game_mousse import *
from random import choice, randint

def cal_pop_fitness_mousse(pop):
    # calculating the fitness value by playing a game with the given weights in chromosome
    fitness = []
    for i in range(pop.shape[0]):
        fit = run_game_with_ML_mousse(display,clock,pop[i])
        print('fitness value of chromosome '+ str(i) +' :  ', fit)
        fitness.append(fit)
    return np.array(fitness)

def select_mating_pool_mousse(pop, fitness, num_parents):
...
def crossover_mousse(parents, offspring_size):
...
def mutation_mousse(offspring_crossover):
