...
for generation in range(num_generations):
    ...
    #2    print('#######  fittest chromosome in gneneration mousse ' + str(generation) +' is having fitness value:  ', np.max(fitness_mousse))
       ...
       # offspring_crossover_mousse = crossover_mousse(parents_mousse, offspring_size=(pop_size_mousse[0] - parents_mousse.shape[0], num_weights_m))
        ...
        #new_population_mousse[0:parents.shape[0], :] = parents_mousse
        ...
        #fitness_mousse=cal_pop_fitness_mousse(new_population_mousse)
        fitness = cal_pop_fitness_mov(new_population)
        print('#######  fittest chromosome mousse moving in gneneration ' + str(generation) +' is having fitness value:  ', np.max(fitness))
    #2    print('#######  fittest chromosome in gneneration mousse ' + str(generation) +' is having fitness value:  ', np.max(fitness_mousse))
        # Selecting the best parents in the population for mating.
        parents = select_mating_pool(new_population, fitness, num_parents_mating)
       # parents_mousse=select_mating_pool_mousse(new_population_mousse, fitness_mousse, num_parents_mating)
   ...
   # new_population_mousse[parents.shape[0]:, :] = offspring_mutation_mousse
        if generation == 399:
            print(generation)
            file_s = open("snake.txt", "w")
            i =  maxf(fitness)
            file_s.write(str(get_w(new_population[i])))
            file_s.close()
